...
def render_layer(
    ...
    gb_pos, _ = interpolate(mesh.v_pos[None, ...], rast_out_s, mesh.t_pos_idx.int()) #[1,512,512,3]

    # Compute geometric normals. We need those because of bent normals trick (for bump mapping)
    v0 = mesh.v_pos[mesh.t_pos_idx[:, 0], :] #[vector.shape, 3], eg[792,3]
    v1 = mesh.v_pos[mesh.t_pos_idx[:, 1], :]
    v2 = mesh.v_pos[mesh.t_pos_idx[:, 2], :]
    face_normals = util.safe_normalize(torch.cross(v1 - v0, v2 - v0)) #[vector.shape, 3], eg[792,3]
    ...
    gb_normal, _ = interpolate(mesh.v_nrm[None, ...], rast_out_s, mesh.t_nrm_idx.int()) #torch.Size([1, 512, 512, 3])
    gb_tangent, _ = interpolate(mesh.v_tng[None, ...], rast_out_s, mesh.t_tng_idx.int()) # Interpolate tangents torch.Size([1, 512, 512, 3])
...
def render_mesh(
    ...
    out = util.avg_pool_nhwc(accum_col, spp) if spp > 1 else accum_col #torch.Size([1, 512, 512, 3])
