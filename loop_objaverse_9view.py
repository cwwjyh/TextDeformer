...
def loop(cfg):
    ...
    if cfg.resize_method == 'cubic':
     ...
     #cww add
    # 在循环开始之前，加载参考图像
    # 假设参考图像存储在 cfg.reference_image_dir 目录下
    reference_image_dir = cfg.reference_image_dir
    reference_images = {}

    view_mapping = {
        0: 'front',  # 0.png 对应 front view
        1: 'back',   # 1.png 对应 back view
        2: 'right',  # 2.png 对应 right view
        3: 'left',   # 3.png 对应 left view
        4: 'front-right',  # 0.png 对应 front view
        5: 'front-left',   # 1.png 对应 back view
        6: 'back-right',  # 2.png 对应 right view
        7: 'back-left',   # 3.png 对应 left view
        8: 'top'   # 3.png 对应 left view
    }

    # 加载参考图像
    for idx, view in view_mapping.items(): #iew_mapping.items()=dict_items([(0, 'front'), (1, 'back'), (2, 'right'), (3, 'left')])
        reference_image_path = os.path.join(reference_image_dir, f'{idx}.png')
        reference_image = Image.open(reference_image_path).convert('RGB')
        reference_image = transforms.ToTensor()(reference_image).unsqueeze(0).to(device)
        # reference_image = resize(reference_image, out_shape=(224, 224), interp_method=resize_method)
        # reference_images[view] = reference_image.repeat(20, 1, 1, 1)  # 重复为 [25, 3, 224, 224]
        reference_images[view] = reference_image  

    # breakpoint()
    print("reference_image.shape:", reference_image.shape)  # 检查形状，应为 (1, 3, 224, 224)
    ...
    for it in t_loop:
        ...
        train_render_original=train_render
        print("train_render_original.shape=", train_render_original.shape)
        ...
        if it % cfg.log_interval_im == 0:
            ...
            s_log = train_render[9:15, :, :, :] #从训练渲染中提取选定前6张图像。
        ...
        # 初始化 mask，用于过滤出需要计算 L2 损失的样本
        l2_mask = (index < 9)  # 前 4 张图片的 mask  # 只对 index=0,1,2,3 的样本计算 L2 损失
        # clip_mask = (index>=4)
        reference_image_batch = torch.zeros_like(train_render_original)  # 形状与 train_render 相同
        # breakpoint()

    # 为前 4 张图片选择对应的参考图像
        for idx in range(9):  # 遍历 0,1,2,3
            idx_mask = (index == idx)  # 当前 index 对应的 mask
            if idx_mask.any():  # 如果有样本的 index 等于当前 idx
                # 从 reference_images 中选择一个子集
                reference_image_batch[idx_mask] = reference_images[view_mapping[idx]][:idx_mask.sum()]  # 选择前 idx_mask.sum() 个样本
        # breakpoint()

         # 计算 L2 损失（前 4 张图片）
        if l2_mask.any():
            l2_loss = torch.nn.functional.mse_loss(train_render_original[l2_mask], reference_image_batch[l2_mask])
        ...
        if it % cfg.log_interval_im == 0:  # 使用与其他图像相同的保存间隔
            ...
            # 获取被mask选中的图像
            masked_reference = reference_image_batch[l2_mask]
            masked_render = train_render_original[l2_mask]
        ...
        normalized_clip_render = (train_render[9:20, :, :, :] - clip_mean[None, :, None, None]) / clip_std[None, :, None, None] #对形变mesh渲染图片进行归一化
        normalized_clip_render_original = (train_render - clip_mean[None, :, None, None]) / clip_std[None, :, None, None] #计算consistency loss的时候，整体计算
        image_embeds = model.encode_image( #形变mesh渲染图片进行clip编码为image embedding
            normalized_clip_render
        )
        with torch.no_grad():
            normalized_base_render = (base_render[9:20, :, :, :] - clip_mean[None, :, None, None]) / clip_std[None, :, None, None]
        ...
        # curr_vp_map = get_vp_map(final_mesh.v_pos, params_camera['mvp'][4:20], 224) # modify
        # for idx, rast_faces in enumerate(train_rast_map[4:20, :, :, 3].view(cfg.batch_size-4, -1)):
        ...
        patch_feats = fe(normalized_clip_render_original) #cww 对所有图片计算patch loss
        ...
        # params_camera['elev'] = params_camera['elev'][4:20]
        # params_camera['azim'] = params_camera['azim'][4:20]
