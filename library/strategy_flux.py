...
class FluxTextEncodingStrategy(TextEncodingStrategy):
    ...
    def encode_tokens(
        ...
        # if apply_t5_attn_mask is None: #这个注释掉会对整体有影响吗 cww 没有影响，因为在预测噪声的时候还是将apply_t5_attn_mask=None
        ...
        t5_attn_mask = tokens[2] if len(tokens) > 2 else None #t5_attn_mask的值全是1，有待检查 cww
        ...
        if t5xxl is not None and t5_tokens is not None:
            ...
            attention_mask = None if not apply_t5_attn_mask else t5_attn_mask.to(t5xxl.device) #attention_mask=None
            t5_out, _ = t5xxl(t5_tokens.to(t5xxl.device), attention_mask, return_dict=False, output_hidden_states=True) #这里耗时5min,因为clip_l.device=t5xxl.device=cpu, 将其移动到cuda上，速度就解决了
