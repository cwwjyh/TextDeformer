...
def prepare_scheduler_for_custom_training(noise_scheduler, device):
    if hasattr(noise_scheduler, "all_snr"):
        if not hasattr(noise_scheduler, "sigmas"):
            print("add sigmas into noise_scheduler")
            # 如果已经有 all_snr 但没有 sigmas，添加 sigmas
            alphas_cumprod = noise_scheduler.alphas_cumprod
            sigmas = ((1 - alphas_cumprod) / alphas_cumprod) ** 0.5
            noise_scheduler.sigmas = sigmas.to(device)
        return
    ...
    noise_scheduler.sigmas = sigma.to(device)  # 直接使用已计算的 sigma
