...
class DreamBoothDataset(BaseDataset):
    ...
    def __init__(
        ...
        def load_dreambooth_dir(subset: DreamBoothSubset):
            ...
            if use_cached_info_for_subset:
                ...
                if strategy is not None:
                    logger.info("get image size from name of cache files") #cww runing
            ...
            logger.info(f"found directory {subset.image_dir} contains {len(img_paths)} image files") #cww runing
        ...
        logger.info(f"{num_train_images} train images with repeating.") #cww running
...
def read_config_from_file(args: argparse.Namespace, parser: argparse.ArgumentParser):
    ...
    logger.info(f"Loading settings from {config_path}...") #cww runing
...
def _load_target_model(args: argparse.Namespace, weight_dtype, device="cpu", unet_use_linear_projection_in_v2=False):
    ...
    if load_stable_diffusion_format: #runing
