...
def attention(q: Tensor, k: Tensor, v: Tensor, pe: Tensor, attn_mask: Optional[Tensor] = None) -> Tensor:
    ...
    x = rearrange(x, "B H L D -> B L (H D)") #torch.Size([1, 273, 3072])
...
class Flux(nn.Module):
    ...
    def forward(
        ...
        img = self.img_in(img) #torch.Size([1, 196, 3072])
        ...
        txt = self.txt_in(txt) #torch.Size([1, 77, 4096])

        ids = torch.cat((txt_ids, img_ids), dim=1) #torch.Size([1, 273, 3])
        ...
        if not self.blocks_to_swap:
            for block_idx, block in enumerate(self.double_blocks): #block是DoubleStreamBlock的网络结构
                img, txt = block(img=img, txt=txt, vec=vec, pe=pe, txt_attention_mask=txt_attention_mask) #jump _forward()
