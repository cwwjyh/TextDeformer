...
class StableDiffusionGuidance(Guidance):
    ...
    def __call__(
        self,
        prompt: str,
        image: Float[torch.Tensor, "1 num_channel height width"] = None, #1改成了4
        latent: Optional[Float[torch.Tensor, "1 4 64 64"]] = None,  #1改成了4
        cfg_scale: float = 7.5,
    ...
    def compute_img_grad(
        self,
        latent: Float[torch.Tensor, "1 4 64 64"], #1改成了4
        prompt: List[str],
        cfg_scale: float,
    ...
    def _compute_img_grad_sds(
        ...
        cfg_scale: float,
        step: Int64[torch.Tensor, "1"],
    ) -> Float[torch.Tensor, "1 4 64 64"]:#1改成了4
        """
    ...
    def edit_img(
        self,
        prompt: str,
        img_init: Float[torch.Tensor, "1 num_channel height width"], #1改成了4
        cfg_scale: float = 7.5,
        start_t: int = 500,
        num_inf_step: int = 20,
    ...
    def encode_images(
        self,
        image: Float[torch.Tensor, "1 3 512 512"] #1改成了4
    ) -> Float[torch.Tensor, "1 4 64 64"]: #1改成了4
        """
    ...
    def decode_latents(
        self,
        latent: Float[torch.Tensor, "1 4 64 64"], #1改成了4
    ) -> Float[torch.Tensor, "1 3 512 512"]: #1改成了4
        """
