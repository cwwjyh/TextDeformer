...
def loop(cfg):
    ...
    for it in t_loop:
        ...
        train_render_original=train_render  #之前的4 view l2 loss是将resize之后的train_render与reference image进行l2 loss
        ...
        l2_mask = (index < 4)  # 前 4 张图片的 mask  # 只对 index=0,1,2,3 的样本计算 L2 损失
        # clip_mask = (index>=4)
        reference_image_batch = torch.zeros_like(train_render_original)  # 形状与 train_render 相同
        # breakpoint()

    # 为前 4 张图片选择对应的参考图像
        for idx in range(4):  # 遍历 0,1,2,3
        ...
        normalized_clip_render = (train_render[4:20, :, :, :] - clip_mean[None, :, None, None]) / clip_std[None, :, None, None] #对形变mesh渲染图片进行归一化
        ...
        with torch.no_grad():
            normalized_base_render = (base_render[4:20, :, :, :] - clip_mean[None, :, None, None]) / clip_std[None, :, None, None]
