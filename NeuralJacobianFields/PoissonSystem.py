...
class SparseMat:
    ...
    def to_coo(self):
        # return coo_matrix((self.vals, (self.inds[0,:], self.inds[1,:])), shape = (self.n, self.m)) # original
        return coo_matrix((self.vals.cpu(), (self.inds.cpu()[0,:], self.inds.cpu()[1,:])), shape = (self.n, self.m)) #cww modify
...
class PoissonSolver:
    ...
    def solve_poisson(self,jacobians): 
        # st = time.time()
        assert(len(jacobians.shape) == 4) #确保jacobians是一个四维张量
        assert(jacobians.shape[2] == 3 and jacobians.shape[3] == 3) #确保最后两个维度大小为3.表示么个雅可比矩阵是3x3的
        ...
        if self.my_splu is None:
            if isinstance(self.lap,SparseMat):
                ...
                #     self.my_splu = self.lap.to_cholesky() #run
                # else:
            ...
            # self.my_splu = scipy_splu(self.lap) #cww modify
        ...
        c = torch.mean(sol, axis=1).unsqueeze(1)  ## Beware the predicted mesh is centered here. c为求解结果的中心点
        # print(f"time for poisson: {time.time() - st}" )
        return sol - c  #返回结果是相对于中心的
...
#该函数的主要目的是将输入的雅可比矩阵（jacobians）重排为一个新的张量 P，以便后续处理
