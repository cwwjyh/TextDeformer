...
parser.add_argument("--output_folder", type=str, default="/views_whole_sphere-test2")
parser.add_argument(
...
map_range.inputs['From Max'].default_value = 4.0  # 根据场景调整这个值
...
def load_object(object_path: str) -> None:
    ...
    elif object_path.endswith(".usd"):
        bpy.ops.import_scene.usd(filepath=object_path)
    elif object_path.endswith(".stl"):
        bpy.ops.import_mesh.stl(filepath=object_path)
    elif object_path.endswith(".usda"):
        bpy.ops.import_scene.usda(filepath=object_path)
    elif object_path.endswith(".dae"):
        bpy.ops.wm.collada_import(filepath=object_path)
    elif object_path.endswith(".abc"):
        bpy.ops.wm.alembic_import(filepath=object_path)
...
def megfile_download_obj(object_url: str, object_uid:str) -> str:
    """Download the object and return the path."""
    # uid = uuid.uuid4()
    uid = object_uid
...
def render_and_save(view_id, object_uid, len_val, azimuth, elevation, distance, ortho=False):
    ...
    if not os.path.exists(os.path.join(args.output_folder, object_uid, 'image', f"{view_id:03d}.png")):
    ...
    if os.path.exists(os.path.join(args.output_folder, object_uid, 'normal', f"{view_id:03d}0001.exr")):
        normal = cv2.imread(os.path.join(args.output_folder, object_uid, 'normal', f"{view_id:03d}0001.exr"), cv2.IMREAD_UNCHANGED)
        normal_unit16 = (normal * 65535).astype(np.uint16)
        cv2.imwrite(os.path.join(args.output_folder, object_uid, 'normal', f"{view_id:03d}.png"), normal_unit16)
        os.remove(os.path.join(args.output_folder, object_uid, 'normal', f"{view_id:03d}0001.exr"))
    ...
    para_path = os.path.join(args.output_folder, object_uid, 'camera', f"{view_id:03d}.npy")
    # np.save(RT_path, RT)
    paras = {}
...
def save_images(object_file: str) -> None:
    ...
    os.makedirs(os.path.join(os.path.basename(args.output_folder), object_uid), exist_ok=True)
    local_dir = os.path.basename(args.output_folder)
    reset_scene()
...
#     bproc.init()
    ...
    normal_output_path = os.path.join(local_dir, object_uid, f"normal_")
    normal_file_output.file_slots[0].path = normal_output_path
    ...
    for j in range(len(camera_locations)):
        view = f"{j:05d}"
        os.makedirs(os.path.join(local_dir, object_uid, view), exist_ok=True)
        depth_output_path = os.path.join(local_dir, object_uid, view, f"depth_{view}")
        depth_file_output.format.file_format = 'PNG'
        depth_file_output.base_path = ""
        depth_file_output.file_slots[0].path = depth_output_path

        bpy.context.view_layer.update()
        camera = build_camera(camera_locations[j][0], camera_locations[j][1], camera_locations[j][2])
        camera.data.type = 'PERSP'
        
        render_path = os.path.join(local_dir, object_uid, view, f"rgb_{view}.png")
        scene.render.filepath = render_path
        ...
        if os.path.exists(os.path.join(local_dir, object_uid, f"normal_0001.exr")):
            normal = cv2.imread(os.path.join(local_dir, object_uid, f"normal_0001.exr"), cv2.IMREAD_UNCHANGED)
            normal_unit16 = (normal * 65535).astype(np.uint16)
            cv2.imwrite(os.path.join(local_dir, object_uid, view, f"normal_{view}.png"), normal_unit16)
            os.remove(os.path.join(local_dir, object_uid, f"normal_0001.exr"))
        ...
        if args.output_folder.startswith("s3:"):
            megfile.s3_upload('{}/{}/{}/rgb_{}.png'.format(local_dir, object_uid, view, view),
                                '{}/{}/{}/rgb_{}.png'.format(args.output_folder, object_uid, view, view))
            megfile.s3_upload('{}/{}/{}/normal_{}.png'.format(local_dir, object_uid, view, view),
                                '{}/{}/{}/normal_{}.png'.format(args.output_folder, object_uid, view, view))
            megfile.s3_upload('{}/{}/{}/depth_{}0001.png'.format(local_dir, object_uid, view, view),
                                '{}/{}/{}/depth_{}.png'.format(args.output_folder, object_uid, view, view))
            os.remove('{}/{}/{}/rgb_{}.png'.format(local_dir, object_uid, view, view))
            os.remove('{}/{}/{}/normal_{}.png'.format(local_dir, object_uid, view, view))
            os.remove('{}/{}/{}/depth_{}0001.png'.format(local_dir, object_uid, view, view))
        else:
            # os.rename('{}/{}/{}/rgb_{}0001.png'.format(local_dir, object_uid, view, view), '{}/{}/{}/rgb_{}.png'.format(local_dir, object_uid, view, view))
            # os.rename('{}/{}/{}/normal_{}0001.png'.format(local_dir, object_uid, view, view), '{}/{}/{}/normal_{}.png'.format(local_dir, object_uid, view, view))
            os.rename('{}/{}/{}/depth_{}0001.png'.format(local_dir, object_uid, view, view), '{}/{}/{}/depth_{}.png'.format(local_dir, object_uid, view, view))
        ...
        if args.output_folder.startswith("s3:"):
            local_RT_path = os.path.join(local_dir, object_uid, view, "RT.txt")
            local_K_path = os.path.join(local_dir, object_uid, view, "K.txt")
            RT_path = os.path.join(args.output_folder, object_uid, view, "RT.txt")
            K_path = os.path.join(args.output_folder, object_uid, view, "K.txt")
            np.savetxt(local_RT_path, RT)
            np.savetxt(local_K_path, K)
            megfile.s3_upload(local_RT_path, RT_path)
            megfile.s3_upload(local_K_path, K_path)
            os.remove(local_RT_path)
            os.remove(local_K_path)
        else:
            RT_path = os.path.join(args.output_folder, object_uid, view, "RT.txt")
            K_path = os.path.join(args.output_folder, object_uid, view, "K.txt")
        ...
        if args.output_folder.startswith("s3:"):
            local_scale_path = os.path.join(local_dir, object_uid, "scale_offset_matrix.txt")
            Scale_path = os.path.join(args.output_folder, object_uid, "scale_offset_matrix.txt")
            np.savetxt(local_scale_path, [scale]+list(offset)+[args.delta_x, args.delta_y, args.delta_z])
            megfile.s3_upload(local_scale_path, Scale_path)
            os.remove(local_scale_path)
        else:
            Scale_path = os.path.join(args.output_folder, object_uid, "scale_offset_matrix.txt")
...
if __name__ == "__main__":
    ...
    if args.object_path.startswith("http") or args.object_path.startswith("s3"):
