...
def loop(cfg):
    ...
    for it in t_loop:
        ...
        # clip_mask = (index>=4)
        reference_image_batch = torch.zeros_like(train_render)  # 形状与 train_render 相同
        # breakpoint()
    ...
    # 为前 4 张图片选择对应的参考图像
        ...
        # breakpoint()
         ...
         # 计算 L2 损失（前 4 张图片）
        if l2_mask.any():
            l2_loss = torch.nn.functional.mse_loss(train_render[l2_mask], reference_image_batch[l2_mask])
        ...
        if it % cfg.log_interval_im == 0:  # 使用与其他图像相同的保存间隔
            ...
            masked_render = train_render[l2_mask]
